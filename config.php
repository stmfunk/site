<?php
   $usersCreate = "CREATE TABLE users (id MEDIUMINT AUTO_INCREMENT NOT NULL, PRIMARY KEY(id), name VARCHAR(50), username VARCHAR(100) UNIQUE NOT NULL, url VARCHAR(80) DEFAULT \"#\")";
   $articlesCreate = "CREATE TABLE articles (id MEDIUMINT AUTO_INCREMENT NOT NULL, PRIMARY KEY(id), title VARCHAR(100) NOT NULL, author VARCHAR(100), date DATETIME NOT NULL, section VARCHAR(100), content TEXT, FOREIGN KEY (section) REFERENCES sections(name), FOREIGN KEY (author) REFERENCES users(username))";
   $articlesCreate = "CREATE TABLE articles (id MEDIUMINT AUTO_INCREMENT NOT NULL, PRIMARY KEY(id), title VARCHAR(100) NOT NULL, author VARCHAR(100), date DATETIME NOT NULL, section VARCHAR(100), content TEXT, FOREIGN KEY (section) REFERENCES sections(name), FOREIGN KEY (author) REFERENCES users(username))";
